...
class CBAMModule(nn.Module):
    ...
    def forward(self, x,return_att=True):
        ...
        self.channel_att_map = channel_att.detach().cpu().squeeze()
        x = x * channel_att
        ...
        self.spatial_att_map = spatial_att.detach().cpu().squeeze()
        x = x * spatial_att
...
class SeModule(nn.Module):
    def __init__(self, in_size, reduction=4):
        ...
        self.attention_map = None  # 用于保存注意力权重

    def forward(self, x):
        weight = self.se(x)
        self.attention_map = weight.detach().cpu().squeeze()  # 保存注意力图
...
class Block(nn.Module):
    ...
    def __init__(self, kernel_size, in_size, expand_size, out_size, act, se, stride):
        ...
        self.cbam = CBAMModule(expand_size)
...
class MobileNetV3_Small(nn.Module):
    def __init__(self, num_classes=94, act=nn.Hardswish):
    ...
    def forward(self, x):
        ...
        features['conv1'] = out  # 保存 conv1 层的输出

        out = self.bneck(out)
        features['bneck'] = out  # 保存 bneck 层的输出

        out = self.hs2(self.bn2(self.conv2(out)))
        features['conv2'] = out  # 保存 conv2 层的输出
